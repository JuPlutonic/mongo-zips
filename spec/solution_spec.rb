...
describe Solution do
  ...
  let!(:connection) { db_url.use('zips_development') }
  ...
  # test for presence of solution.rb requiring mongo gem and defining a class Solution
  context "rq02" do
    it "all work should be in solution.rb file" do
      expect(File.exists?("solution.rb")).to be true
    end
    it "solution.rb file imports the mongo gem" do
      expect(includes_gem?("solution.rb", "mongo")).to be true
    end
    it "Class Solution should exist in solution" do
  ...
  context "rq03" do
    ...
    it "returns a mongo client connected to zips_development collection" do
      expect(db).to_not be_nil
      expect(db).to be_a(Mongo::Client)
      expect(db.database.name).to eq('zips_development')
    end
